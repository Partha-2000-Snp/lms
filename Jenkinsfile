        ...
        stage('Release LMS') {
            steps {
                script {
                    ...
                    echo "${Version}"
                    sh 'zip webapp/lms-${Version}.zip -r webapp/dist && pwd && ls'
        ...
        stage('Deploy LMS') {
            steps {
                script {
                    ...
                    def Version = packageJson.version
                    echo "${Version}"
                    sh "curl -u admin:123 -X GET \'ttp://52.149.182.114:8081/repository/lms-front/lms-${Version}.zip\' --output lms-'${Version}'.zip"
